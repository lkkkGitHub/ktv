...
document.write('<script src="../../static/regist/pictureYanZheng/js/jquery.idcode.js"><\/script>');
...
function GetTel() {
    ...
    $("#xianshiEmail").html(str);
}

//获取用户名
function GetUserName() {
    var str = document.getElementById("uname").value;
    $("#xianshisuccess").html("恭喜" + str + "，您已注册成功，现在开始您的音乐之旅吧！");
}
...
function CheckUserName() {
    var User;
    $.ajax({
        url: "/user/verifyName", //(默认: 当前页地址) 发送请求的地址
...
//             var flag = json;
//             if (flag == true) {
//                 $("#successEmail").html("验证码已发送至您的邮箱，请输入邮件中的验证码，确保您的邮箱地址真实有效。");
//             } else if (flag == false) {
//                 $("#successEmail").html("发送失败，请重新发送，确保您的邮箱号码真实有效。");
//             }
//         },
//         error: function (XMLHttpRequest, textStatus, errorTh) {
//             console.log(textStatus)
...
function saveData() {
    $.ajax({
            url: "/user/register", //(默认: 当前页地址) 发送请求的地址
        type: "post", //(默认: "get") 请求方式 ("post" 或 "get")， 默认为 "get"。注意：其它 http请求方法，如 put和 delete也可以使用，但仅部分浏览器支持。
        async: false,//(默认: true) 默认设置下，所有请求均为异步请求。如果需要发送同步请求，请将此选项设置为 false。注意，同步请求将锁住浏览器，用户其它操作必须等待请求完成才可以执行。
        contentType: "application/x-www-form-urlencoded",//(默认: "application/x-www-form-urlencoded") 发送信息至服务器时内容编码类型。默认值适合大多数应用场合。
        data: {
...
//			randCode:'',
        ...
        this.rules = {
            ...
            isRandCode: function (value, errorMsg) {
                errorMsg = errorMsg || " ";
                if (IsBy != value) return errorMsg;
            },
            isUserNameYan: function (value, errorMsg) {
                //alert(User);
                var User = CheckUserName();
                errorMsg = errorMsg || " ";
                if (User == false) return errorMsg;
            },
            isEmailBindingYan: function (value, errorMsg) {
                //alert(User);
                var emailStr = emailBinding();
                errorMsg = errorMsg || " ";
                if (emailStr == false) return errorMsg;
            },
